...
- **Slice 2.5 — LẠM PHÁT** (chốt 24/07/2026, làm ngay sau Slice 2): `inflationAnnualRate` trong scenario config (VN ~3,5%/năm), áp MỖI TẾT (khớp Recap): chi phí sống + trọ + gửi về quê ×(1+rate). Lương tăng theo nhưng THẤP hơn (~3%/năm) — khoảng hở là sức ép phải đầu tư. Cổ tức per-unit trượt theo lạm phát (giữ đích thắng khả thi); vàng/đất phòng thủ qua drift giá. Bài học: tiền mặt nằm im là tiền đang chết. **Gate mới: bot Ôm-Tiền-Mặt (không bao giờ đầu tư) phải thua poorAtRetirement.** Cảnh báo: mục tiêu di động → tune lại toàn bộ baseline + 5 gate bot.
